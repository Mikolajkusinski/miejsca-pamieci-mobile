...
class _TrailDetailsState extends State<TrailDetails> {
  ...
  void initState() {
    super.initState();
    try {
      fetchTrail(context, widget.trailId).then((trail) {
        _trail = trail;
        _updatedImages = _trail.images!.map((image) {
  ...
  Future<void> _launchMaps(BuildContext context) async {
    final url = Uri.parse(ApiConstants.googleSearchByLatLng(
        _trail.coordinates[0].latitude, _trail.coordinates[0].longitude));
  ...
  Future<void> _launchWikipedia(BuildContext context) async {
    final url = Uri.parse(_trail.wikiLink);
    if (await canLaunchUrl(url)) {
      await launchUrl(url);
    } else {
  ...
  Future<void> _launchTopicPage(BuildContext context) async {
    final url = Uri.parse(_trail.topicLink);
    if (await canLaunchUrl(url)) {
      await launchUrl(url);
    } else {
      showErrorToast(LocaleKeys.link_error.tr());
                          ...
                          children: [
                            ...
                            Text(
                              LocaleKeys.type_info.tr(
                                  namedArgs: {'type': _trail.typeValue.tr()}),
                              style: const TextStyle(fontSize: 18),
                            ),
                            Text(
                              LocaleKeys.period_info.tr(namedArgs: {
                                'period': _trail.periodValue.tr()
                              }),
                              style: const TextStyle(fontSize: 18),
                            ),
                            Text(
                              LocaleKeys.username_info
                                  .tr(namedArgs: {'username': _trail.username}),
                              style: const TextStyle(fontSize: 18),
                            ),
                            Text(
                              LocaleKeys.date_info
                                  .tr(namedArgs: {'date': _trail.creationDate}),
                              style: const TextStyle(fontSize: 18),
                            ),
                          ],
                        ),
